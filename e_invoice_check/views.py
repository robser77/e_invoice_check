...
from e_invoice_check.helpers import validate_file_content, use_xslt_proc_2, use_xslt_proc_3, Xslt_proc
        ...
        for uploaded_file in request.files.getlist('file'):
            ...
            if filename != '':
                ...
                if file_ext not in current_app.config['UPLOAD_EXTENSIONS']:
                    flash(f"File extension not allowed. Allowed values: xml \n Filename: {filename}") 
                    # TODO add custom 400
                    abort(400)
                ...
                stylesheet = etree.parse("e_invoice_check/static/xslt/stylesheet-ubl.xslt")
                xslt = etree.tostring(stylesheet, pretty_print="True", encoding="unicode") 

                # create html view of the invoice and save to tmeplate dir
                doc_template_name = "document.html"

                xslt_proc = Xslt_proc(stylesheet_text=xslt)
                doc_template_html = use_xslt_proc_3(xslt_proc, xslt, xml, '')

                with open(f"e_invoice_check/templates/views/{doc_template_name}", "w") as html_file:
                    html_file.write(doc_template_html)
                
                return render_template("views/doc-view.html", filename=filename, doc_template_name=doc_template_name)
